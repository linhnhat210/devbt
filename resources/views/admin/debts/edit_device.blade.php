...
@section('title', 'Chỉnh sửa công nợ tiền thiết bị')
        ...
        <div class="container-lg px-4">
            ...
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <strong>Chỉnh sửa công nợ tiền thiết bị "{{ $debt->debt_code }}"</strong>
                    <span class="badge bg-warning text-dark">{{ $debt->status }}</span>
                </div>

                <div class="card-body">
                    <form action="{{ route('admin.debts.update', $debt->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" value="{{ $debt->project->name ?? '---' }}" disabled>

                                <label class="form-label mt-3">Loại công nợ</label>
                                <input type="text" class="form-control" value="Tiền thiết bị" disabled>

                                <label class="form-label mt-3">Thời gian bắt đầu thu phí</label>
                                <input type="text" name="start_date" class="form-control datepicker"
                                    value="{{ old('start_date', $debt->devicePayment->start_date ?? '') }}">
                                @error('start_date')
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <input type="text" class="form-control"
                                        value="{{ number_format($debt->project->category->internal_price ?? 0, 0, ',', '.') }}"
                                        disabled>
                                    <span class="input-group-text">VNĐ</span>
                                </div>

                                <label for="amount" class="form-label mt-3"><span class="text-danger">*</span> Giá duyệt</label>
                                <div class="input-group">
